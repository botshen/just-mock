...
const resolve = <D extends { id: string }>(maybeString: string | VNodeChild | ((column: Column<D>, data: D[]) => VNodeChild), column: Column<D>, data: D[]) => {
  return typeof maybeString === 'function' ? maybeString(column, data) : maybeString
}
export const CreateTable = <D extends { id: string }>() => createComponent<TableOptions<D>>({
  props: {
    list: null,
    store: null,
    columns: [],
      ...
      {(props.leftActions || props.rightActions || props.multiSelect) && (
        <MergeClass tag="x-table-actions" baseClass="flex" class={props.actionsClass}>
          ...
          {props.multiSelect && (
            mode.value === 'normal'
              ? (
                  <Button2 level="text" width="fit" class="h-8 text-sm font-bold"
              ...
              : (
                  <Button2 level="normal" width="fit" class="h-8 text-sm font-bold"
      ...
      <div class="relative min-h-[200px]">
        {
          props.list?.[0]
            ? (
                <x-table class="overflow-x-auto w-full relative"
                 style={{
                  display: 'grid',
                  gridTemplateColumns: 
                  [mode.value === 'select' ? 'max-content' : '',
                    ...props.columns.map(column => column[2]?.width ?? 'max-content')].join(' '),
                }}
                >
                  {/* head */}
                  ...
                  {props.list.map((row, rowIndex) => (
                    <>
                      ...
                      {/* 勾选 */}
                      {mode.value === 'select' && (
                        <x-cell class={mc(finalCellClass.value, 'w-8 px-2 py-4 whitespace-nowrap text-sm', rowIndex % 2 === 0 ? 'even' : 'odd')}>
                          <Checkbox
                      ...
                      {props.columns.map(column => (
                        <x-cell class={mc(finalCellClass.value, column[2]?.class, rowIndex % 2 === 0 ? 'even' : 'odd')}>
